...
from pyalgotrade.poloniex import livebroker
...
class BacktestingBroker(backtesting.Broker):
    """A poloniex backtesting broker.

    :param cash: The initial amount of BASE TOKEN.
    :type cash: int/float.
    ...
    def submitOrder(self, order):
        if order.isInitial():
            # Override user settings based on poloniex order defaults
    ...
    def createLimitOrder(self, action, instrument, limitPrice, quantity):
        if instrument != common.INSTRUMENT_TOKEN:
            raise Exception("Only %s instrument is supported" % common.INSTRUMENT_TOKEN)
        ...
        if action == broker.Order.Action.BUY:
            ...
            cashTokenRequired = limitPrice * quantity + fee
            if cashTokenRequired > self.getCash(False):
                raise Exception("Not enough %s" % common.CASH_TOKEN)
        elif action == broker.Order.Action.SELL:
            # Check that there are enough coins.
            if quantity > self.getShares(common.INSTRUMENT_TOKEN):
                raise Exception("Not enough %s" % common.INSTRUMENT_TOKEN)
        else:
...
class PaperTradingBroker(BacktestingBroker):
    """A poloniex paper trading broker.
