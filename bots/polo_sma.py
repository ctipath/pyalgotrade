    ...
    def __replaceBestOrder(self, orderBookModificationOrRemoval):
        if orderBookModificationOrRemoval.isBid():
            ...
            if bestBid is not None and self.__bid != bestBid:
                self.__bid = bestBid
                self.info("Order book updated. Best (highest) bid is now %s" % (self.__bid))
        else:
            ...
            if bestAsk is not None and self.__ask != bestAsk:
                self.__ask = bestAsk
                self.info("Order book updated. Best (lowest) ask is now %s" % (self.__ask))
    ...
    def onBars(self, bars):
        ...
        if self.__position is None:
            if cross.cross_above(self.__prices, self.__sma) > 0:
                ...
                self.__position = self.enterLongLimit(INSTRUMENT_TOKEN, self.__ask, self.__posSize, True)
